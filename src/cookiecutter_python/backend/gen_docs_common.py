...
def get_docs_gen_internal_config() -> t.Dict[str, str]:
    ...
    doc_builder_id_2_doc_folder: t.Dict[str, str] = {
        ...
        for path in PROJ_TEMPLATE_DIR.glob('docs-*') if path.is_dir()
    }
    assert doc_builder_id_2_doc_folder, f"templated_proj_folder: {PROJ_TEMPLATE_DIR}, with files: {list(PROJ_TEMPLATE_DIR.glob('*'))}"
