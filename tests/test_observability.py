...
def test_docker_compose_observability_file_exists(
    ...
    project_path = generate_test_project(include_observability="yes")
    docker_compose_file = Path(project_path) / 'observability' / 'docker-compose.observability.yml'
...
def test_grafana_datasource_config_exists(
    ...
    # WHEN we generate a project with observability stack
    project_path = generate_test_project(include_observability="yes")
    datasource_config = Path(project_path) / 'observability' / 'grafana' / 'datasources' / 'loki.yml'
...
def test_docker_compose_contains_project_specific_names(
    ...
    assert f'{project_slug}-grafana' in content, f"Should contain project-specific Grafana container name with slug: {project_slug}"
    assert f'{project_slug}-loki' in content, f"Should contain project-specific Loki container name with slug: {project_slug}"
    assert f'{project_slug}-promtail' in content, f"Should contain project-specific Promtail container name with slug: {project_slug}"
...
def test_grafana_datasource_points_to_loki(
    ...
    assert 'loki:3100' in loki_datasource['url'], "Should point to Loki service on standard port"
