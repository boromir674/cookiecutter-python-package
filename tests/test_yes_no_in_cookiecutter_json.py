...
def test_cookiecutter_context_preserves_choice_variable_types(tmp_path: Path):
    ...
    project_template_dir = template_dir / "{{ cookiecutter.project_name|lower|replace(' ', '-') }}"
...
"""
    ...
    assert cookiecutter_vars['multiple_choice'] == ["c1", "c2", "c3"]
...
"""
    ...
    # Additional verification: ensure no automatic boolean conversion happened
    # The yes/no should remain as strings, not be converted to True/False
    # assert cookiecutter_vars['enable_feature'] != True
    # assert cookiecutter_vars['enable_feature'] != False
    # assert cookiecutter_vars['enable_feature'] == 'yes'
