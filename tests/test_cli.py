...
def test_cli(main_command, generate_python_args, isolated_cli_runner):
    ...
    result = isolated_cli_runner.invoke(
        main_command,
        args=args,
        input=None, env=None, catch_exceptions=False, color=False,
        **kwargs,
    )
...
def mock_check_pypi():
    ...
    def get_mock_check_pypi(answer: bool):
        return type('Future', (), {
            'result': lambda: type('HttpResponse', (), {
...
def test_cli_offline(
    get_object, mock_check_pypi, generate_python_args, isolated_cli_runner
):
    args, kwargs = generate_python_args(output_dir='gen')
    cli_main = get_object('main', 'cookiecutter_python.cli')
    get_object('generate', 'cookiecutter_python.backend.main',
